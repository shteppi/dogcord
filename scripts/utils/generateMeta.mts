...
const releases = await fetch("https://api.github.com/repos/shteppi/dogcord/releases", {
    headers: {
...
const metaInfo = await (async () => {
    for (const release of releases) {
        const metaAsset = release.assets.find((a: any) => a.name === "org.dogpack.dogcord.metainfo.xml");
...
await fs.writeFile("./dist/org.dogpack.dogcord.metainfo.xml", output, "utf-8");
console.log("Updated meta information written to ./dist/org.dogpack.dogcord.metainfo.xml 🐕");
