...
export async function askToApplySteamLayout(win: BrowserWindow) {
    ...
    const { response } = await dialog.showMessageBox(win, {
        message: `${update ? "Update" : "Apply"} Dog Cord Steam Input Layout? 🐕`,
        detail: `Would you like to ${update ? "Update" : "Apply"} Dog Cord's recommended Steam Deck controller settings?
